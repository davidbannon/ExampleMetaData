...
//              GetFirst: boolean; KeyList: TStringList=nil): boolean;
            ...
            function ExtractFieldsFromJSON(const JStr: string; out EName, Cat,
...
{$ifndef APPEXMETADATA}     // ExMetaData is a standalone app for editing meta data
uses
    uConst {$ifdef EXTESTMODE}, Main_Examples{$endif} ;
{$else}
const    cExamplesDir='anywhere';
{$endif}
...
//    if Result = '' then                 // Needless noise, normally called when dir not yet present. NOT sent to Lazarus yet
...
function TExampleData.ExtractFieldsFromJSON(const JStr: string;   // Only used by ExMetaData app.
                    out EName, Cat, Keys, Desc, Error: string): boolean;
var
    jData, jItem : TJSONData;
    STL : TStringList;
    St : string;
